...
public:
    vector<int> pathsWithMaxScore(vector<string>& board) {
        ...
        for (int r = n - 1; r >= 0; --r) {
            for (int c = n - 1; c >= 0; --c) {
                if (board[r][c] == 'X' || max_sum[r][c] == -1) {
                    continue;
                }
                
                int current_sum = max_sum[r][c];
                ...
                for (int i = 0; i < 3; ++i) {
                    ...
                    if (nr >= 0 && nc >= 0 && board[nr][nc] != 'X') {
                        ...
                        int next_sum = current_sum + cell_value;
                        
                        if (next_sum > max_sum[nr][nc]) {
                            max_sum[nr][nc] = next_sum;
                            path_count[nr][nc] = current_paths;
                        } else if (next_sum == max_sum[nr][nc]) {
        ...
        return {max_sum[0][0], path_count[0][0]};
    }
};
